...
impl From<&str> for DiscordResponse {
    fn from(value: &str) -> Self {
        DiscordResponse::message_response(value)
    }
}
...
#[cfg(test)]
mod test {
    use crate::response::{DiscordResponse, open_buy_modal};

    #[test]
    fn test_ping() {
        let response = serde_json::to_string(&DiscordResponse::ping_response()).unwrap();
        assert_eq!(&response, r#"{"type":1}"#)
    }
    #[test]
    fn test_simple_response_message() {
        let response =
            serde_json::to_string(&DiscordResponse::message_response("this is a simple message")).unwrap();
        assert_eq!(
            &response,
